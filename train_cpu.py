...
def get_train_data(data,batch_size,time_step,train_end,predict_step,train_begin=0):
    ...
    normalized_train_data = data_train/10.0
...
def get_test_data(data,time_step,test_begin,predict_step):
    ...
    #maxvalue += 1
    #print(maxvalue)
    normalized_test_data = data_test/10.0
...
def train_lstm(data,input_size,output_size,lr,train_time,rnn_unit,weights,biases,train_end,
    ...
    with tf.Session() as sess:
        ...
        for i in range(output_size):
            test_y[:, i] = test_y[:, i] * 10.0  #训练和预测时除了10
            test_predict[:, i] = test_predict[:, i] * 10.0
        #for i in range(output_size):
...
if __name__=="__main__":
    ...
    predict_step = 7 #预测predict_step分钟后的负载
    kp = 1  # dropout保留节点的比例
    smooth = 0  # 为1则在时间维度上平滑数据
    train_time = 0  # 所有数据的训练轮次
    train_end = 500  # 训练集截取到的位置
    filename = 'csv_for_train/grafana_data_export_7-31.csv'
    save_model_path = './save/predict_cpu_7-31/'  # checkpoint存在的目录
