'''
读取predict_save文件夹下的存储点，这样直接使用训练好的网络来预测。
两种调度，一个是把预测会有高负载的移到高性能机器上，一个是从高性能机器上移出region
实时读取数据，每固定时间训练一次，（如果节点关了的话就模拟每分钟读一次文件）
每次训练后都存储模型，
刚开始直接加载训练好的模型
'''
import pandas as pd
...
def get_predict_data(time_step):
    ...
    return mean, std, normalized_test_data
...
def get_train_data(batch_size,time_step,predict_step):
    ...
    std = np.std(history_data, axis=0)
    maxvalue = np.max(history_data, axis=0)
    normalized_train_data = (history_data - mean) / std  # 标准化
...
#——————————————————模型—————————————————
def train_lstm(input_size,output_size,lr,train_time,rnn_unit,weights,biases,train_end,
               batch_size,time_step,kp,url,err_eval_step,predict_step,train_begin=0):
    ...
    with tf.Session() as sess:
        ...
        ckpt = tf.train.get_checkpoint_state('./save/')  # checkpoint存在的目录
        ...
        while True:#每个预测就是一次循环，每隔一段时间就训练一次，存储模型
            ...
            if label > 1000: #值没有意义，只是运行一段时间后不需要用到label,
                ...
                label += 1
            get_line_input(output_size, url)
            print('label:',label)
            if label >= time_step + predict_step:
                train_num += 1
            if label >= time_step:
                mean, std, test_x = get_predict_data(time_step)
                ...
                for i in range(output_size):
                    predict[i] = predict[i] * std[i+1] + mean[i+1] # i+1是因为输出少一维时间
                    maxvalue[i] = maxvalue[i] * std[i + 1] + mean[i + 1]
                    minvalue[i] = minvalue[i] * std[i + 1] + mean[i + 1]
                    if predict[i]<0:
                        predict[i] = 0
            ...
            time.sleep(5) #每隔60s循环一次，实际上因为程序运行所以超过一分钟
...
def start_predict():
    ...
    time_step = 30  # 前time_step步来预测下一步
    ...
    url = 'http://192.168.1.128:2379/pd/api/v1/regions'
    global history_data
    global predict_message
    ...
    train_lstm(input_size,output_size,lr,train_time,rnn_unit,weights,biases,train_end,batch_size,time_step,kp,url,err_eval_step,predict_step)