...
实时读取数据，每固定时间训练一次，
每次训练后都存储模型，
和predict_tikv.py一样，只是加了通过命令来伸缩节点
'''
import numpy as np
...
def train_lstm(input_size,output_size,lr,rnn_unit,weights,biases,batch_size,time_step,kp,
               predict_step,save_model_path,save_model_name,init_tikv_replicas,yaml_path, prome_addr):
...
def start_predict_cpu(prome_addr,yaml_path):
    input_size = 1  # 输入维度
    output_size = 1  # 输出维度
    rnn_unit = 12 # 隐藏层节点
    lr = 0.0004  # 学习率
    ...
    train_lstm(input_size,output_size,lr,rnn_unit,weights,biases,batch_size,
               time_step,kp,predict_step,save_model_path,save_model_name,init_tikv_replicas,yaml_path, prome_addr)
